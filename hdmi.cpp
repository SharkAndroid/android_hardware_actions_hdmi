...
static int open_hdmi(const struct hw_module_t* module, const char* name, struct hw_device_t** device);
 ...
 *
 */
static int _map_hdmi_param_to_drv_id(int width, int height, float refreshrate, int isprogressive, int aspect_ratio) {
...
static int hdmi_set_mode(struct hdmi_device_t *dev, int width, int height, float freshrate, int isprogressive, int aspect_ratio) {
...
static int hdmi_resolve_config(struct hdmi_device_t *dev, video_setting_t *vid_setting) {
    return 0;
}
